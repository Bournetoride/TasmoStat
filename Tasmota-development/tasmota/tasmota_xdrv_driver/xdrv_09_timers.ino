...
uint16_t SunMinutes(uint32_t dawn)
{
  ...
  DuskTillDawn(&hour[0], &minute[0], &hour[1], &minute[1]);
  dawn &= 1;
  return TasmotaGlobal.user_globals[5] = (hour[dawn] *60) + minute[dawn];
}
...
{
        uint8_t hour[2];                                        //for sunset time
        uint8_t minute[2];
        int16_t time_min = 0;
        int16_t min_time = 1440;
        int16_t sunset_min = 0;
        int16_t min_sunset = 1111;
  if (RtcTime.valid) {
    time_min = (RtcTime.hour*60) + RtcTime.minute;                   // + RtcTime.minute Now minute number
      time_min = min(time_min, min_time);
      TasmotaGlobal.min_time = (int16_t)time_min;                            //Now minute number
      DuskTillDawn(&hour[0], &minute[0], &hour[1], &minute[1]);  
      sunset_min = (hour[1]*60) + RtcTime.minute ;                       // + RtcTime.minute  Sunset minute number 
      sunset_min = min(sunset_min, min_sunset);
      TasmotaGlobal.min_sunset = (int16_t)(sunset_min);
